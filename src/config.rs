...
// static lifetime is used here, since a workspace lives as long as the program
pub fn read() -> Config {
    let config_directory = home_directory().join(".config/workspacer");
    let config_file_path = home_directory().join(".config/workspacer/config");
